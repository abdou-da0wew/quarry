...
func (c *Checker) parse(r io.Reader) ([]*Group, error) {
        ...
        // Add the last group (even if it only has crawl-delay and no rules)
        if currentGroup != nil {
                groups = append(groups, currentGroup)
        }
...
func (c *Checker) GetCrawlDelay() time.Duration {
        ...
        // First check for exact user-agent match
        for _, group := range c.groups {
                if group.UserAgent == userAgentLower || strings.Contains(userAgentLower, group.UserAgent) {
                        if group.CrawlDelay > 0 && group.UserAgent != "*" {
...
func matches(path, pattern string) bool {
        ...
        // Handle $ suffix (end of path anchor)
...
// matchWildcard handles patterns containing * wildcards.
func matchWildcard(path, pattern string) bool {
        ...
        if len(parts) > 1 {
                ...
                if lastPart != "" && !strings.HasSuffix(remaining, lastPart) {
                        // Check if remaining ends with lastPart (it should contain it at the end)
                        return strings.HasSuffix(path, lastPart) || strings.Contains(remaining, lastPart)
                }
        }
