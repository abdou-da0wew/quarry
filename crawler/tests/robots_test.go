...
Disallow: /general/`,
        ...
        for _, tt := range tests {
                t.Run(tt.name, func(t *testing.T) {
                        ...
                        for urlPath, expected := range tt.urlChecks {
                                // Use server URL with the path from the test case
                                // Extract path portion: http://example.com/path -> /path
                                pathStart := strings.Index(urlPath, "://")
                                if pathStart == -1 {
                                        t.Errorf("invalid URL: %q", urlPath)
                                        continue
                                }
                                pathStart += 3 // Skip "://"
                                slashIdx := strings.Index(urlPath[pathStart:], "/")
                                if slashIdx == -1 {
                                        fullURL := server.URL + "/"
                                        result := checker.IsAllowed(fullURL)
                                        if result != expected {
                                                t.Errorf("IsAllowed(%q) = %v, expected %v", fullURL, result, expected)
                                        }
                                        continue
                                }
                                path := urlPath[pathStart+slashIdx:]
                                fullURL := server.URL + path
                                result := checker.IsAllowed(fullURL)
                                if result != expected {
                                        t.Errorf("IsAllowed(%q) = %v, expected %v", fullURL, result, expected)
                                }
